...
#include <dtprintf.h>
// Shaders
#include "constantWithBuffer.vs.h"   // 頂点シェーダ
...
int APIENTRY _tWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
	...
	hr = pDevice->CreateVertexShader( g_vs_constantWithBuffer, sizeof( g_vs_constantWithBuffer ), NULL, &pVertexShader );
	if ( FAILED( hr ) )
	{
	...
	hr = pDevice->CreateInputLayout( verticesDesc, 2, g_vs_constantWithBuffer, sizeof( g_vs_constantWithBuffer ), &pInputLayout );
	if ( FAILED( hr ) )
	{
	...
	while ( 1 )
	{
		...
		else
		{
			...
			if ( SUCCEEDED( hr ) )
			{
				...
				float data = sinf( ( count++ % 200 ) * 2.0f * 3.141593f / 200.0f ) * 0.5f + 0.5f;   // 0～1
				*static_cast< float * >( mapped.pData ) = data;
