...
int APIENTRY _tWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
	...
	// メモリ確保・読み込み
	vertices = new float[ 6 * vertices_count ];
	fread( vertices, sizeof( float ) * 6, vertices_count, fp );
	// 面数
	...
	hr = pDevice->CreateVertexShader( g_vs_lambert, sizeof( g_vs_lambert ), NULL, &pVertexShader );
	if ( FAILED( hr ) )
	{
	...
	hr = pDevice->CreateInputLayout( verticesDesc, 2, g_vs_lambert, sizeof( g_vs_lambert ), &pInputLayout );
	if ( FAILED( hr ) )
	{
