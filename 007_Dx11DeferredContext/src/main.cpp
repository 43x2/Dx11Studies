...
int APIENTRY _tWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
	...
	if ( FAILED( hr ) )
	{
		MessageBox( NULL, _T( "失敗: ID3D11Device::CreateDeferredContext()" ), _T( "エラー" ), MB_OK | MB_ICONERROR );
		return 0;
	}
	dtprintf( _T( "ID3D11Device::CreateDeferredContext: ok (pDeferredContext: 0x%p)\n" ), pDeferredContext );
	...
	while ( 1 )
	{
		...
		{
			// レンダーターゲットをバインド
			pDeferredContext->OMSetRenderTargets( 1, &pRenderTargetView, NULL );
			// ビューポートをバインド
			pDeferredContext->RSSetViewports( 1, &viewport );
			// 頂点バッファをバインド
			UINT strides[] = { sizeof( float ) * 7 };
			UINT offsets[] = { 0 };
			pDeferredContext->IASetVertexBuffers( 0, 1, &pVertexBuffer, strides, offsets );
			// インデックスバッファをバインド
			pDeferredContext->IASetIndexBuffer( pIndexBuffer, DXGI_FORMAT_R32_UINT, 0 );
			// プリミティブタイプを設定
			pDeferredContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
			// シェーダをバインド
			pDeferredContext->VSSetShader( pVertexShader, NULL, 0 );
			pDeferredContext->PSSetShader( pPixelShader, NULL, 0 );
			// 入力レイアウトをバインド
			...
			pDeferredContext->ClearRenderTargetView( pRenderTargetView, clear );

			// 描画
			pDeferredContext->DrawIndexed( 12, 0, 0 );


			// ディファードコンテキストのコマンドリストを消化
			ID3D11CommandList * pDeferredCommandList = NULL;
			hr = pDeferredContext->FinishCommandList( TRUE, &pDeferredCommandList );
			if ( SUCCEEDED( hr ) )
			{
				// コマンドリスト再生
				pDeviceContext->ExecuteCommandList( pDeferredCommandList, TRUE );
				COM_SAFE_RELEASE( pDeferredCommandList );
			}
			else
			{
				dtprintf( _T( "ID3D11DeviceContext::FinishCommandList: failed!\n" ) );
			}
	...
	COM_SAFE_RELEASE( pDeferredContext );
